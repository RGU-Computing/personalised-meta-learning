    ...
    def load_data_cache(self, data_pack, mode='train'):
        ...
        for sample in range(10):  # num of episodes
            ...
            for i in range(self.batchsz):  # one batch means one set
                ...
                selected_person_class = np.random.choice(data_pack.shape[0], self.n_way, False)
                for j, cur_class in enumerate(selected_person_class):
                    # meta-training and meta-test
                    x_spt.extend(data_pack[cur_class][:self.k_shot])
                    qry = data_pack[cur_class][self.k_shot:]
                    x_qry.extend(qry)
                ...
                x_spt = np.array(x_spt).reshape(self.n_way * self.k_shot, 80*16)[perm]
                y_spt = np.array(y_spt).reshape(self.n_way * self.k_shot)[perm]
                perm = np.random.permutation(len(x_qry))
                x_qry = np.array(x_qry).reshape(len(perm), 80*16)[perm]
            ...
            # [b, setsz, 1, 80, 16]
            x_spts = np.array(x_spts).astype(np.float32).reshape(self.batchsz, setsz, 80*16)
            y_spts = np.array(y_spts).astype(np.int).reshape(self.batchsz, setsz)
            # [b, qrysz, 1, 80, 16]
            x_qrys_ = np.array(x_qrys_).astype(np.float32).reshape(self.batchsz, min_qry, 80*16)

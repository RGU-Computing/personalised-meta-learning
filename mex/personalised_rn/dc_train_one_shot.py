...
def main():
    ...
    for episode in range(EPISODE):
        ...
        task = tg.DCTask(metatrain_character_folders, CLASS_NUM, SPT_NUM_PER_CLASS)
        ...
        batch_labels = []
        for ind in range(30):
            _samples, _sample_labels = sample_dataloader.__iter__().next()
            print(_samples.shape)
            _batches, _batch_labels = batch_dataloader.__iter__().next()
            samples += _samples
            sample_labels += _sample_labels
            batches += _batches
            batch_labels += _batch_labels

        samples = torch.cat(samples, dim=0)
        samples = samples.view(samples.shape[0], 1, samples.shape[1], samples.shape[2])
        # sample_labels = torch.cat(sample_labels, dim=0)
        batches = torch.cat(batches, dim=0)
        batches = batches.view(batches.shape[0], 1, batches.shape[1], batches.shape[2])
        ...
        if (episode + 1) % 5 == 0:
            ...
            for i in range(TEST_EPISODE):
                task = tg.DCTask(metatest_character_folders, CLASS_NUM, SPT_NUM_PER_CLASS)
